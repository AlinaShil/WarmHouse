...
Ниже представлена ER-диаграмма для новой системы "Теплый дом".
...
1. REST-API для всех пользовательских операций:

    * Взаимодействие UI ↔ API Gateway;
    * Управление сущностями (User, Device, Rule) и интеграции с внешними сервисами (платёжный шлюз).

2. Асинхронные API / Messaging для внутренних рабочих процессов и устройств:

    * MQTT между Command Dispatcher и устройствами (реле, датчики) — гарантирует доставку команд и приём телеметрии даже при кратковременных разрывах связи.
    * Kafka/RabbitMQ между микросервисами (Telemetry Ingest → Rule Engine → Command Dispatcher) — буферизация событий и decoupling.

Такой гибридный подход позволит сохранить простоту и предсказуемость клиентского REST-API, одновременно обеспечив высокую надёжность, масштабируемость и расширяемость внутренних коммуникаций.

### 2. Документация API

OpenAPI спецификации для микросервисов расположены в проекте в папке apps/temperature/api-docs.

# Задание 5. Работа с docker и docker-compose

В папке apps создан новый модуль temperature со своим DockerFile. Модуль добавлен в общий файл проекта docker-compose.
